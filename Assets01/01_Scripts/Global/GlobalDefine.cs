...
namespace Proto_00_N
{
	...
	namespace GlobalDefine
	{
		...
		public static class ObjectData
		{
			...
			{
				[Header("Moveable")]						// 이동 효과
				public float fWeight = 1.0f;				// 무게			: 해당 수치가 높을수록 낮게, 짧게 튕겨남 ( 0일 경우 오류 )
				public float fAirHold = 1.0f;				// 체공			: 해당 수치가 높을수록 오래 튕겨남 ( 0일 경우 오류 )
			}
			...
			{
				[Header("Health")]
				public int iLife = 1;						// 라이프
				public float fHealthMax = 5;				// 최대 체력
				public float fHealthNow = 5;				// 현재 체력

				[Header("Battle")]
				public float fAttackPower = 1;				// 공격력
				public float fDefendPower = 0;				// 방어력
				public float fPropertyAttackPower = 1;		// 속성 공격력 - 나중에 자세한 구현사항 필요

				[Header("Control")]
				public float fAttackSpeed = 1.0f;			// 공격 속도 (1.0 = 100%)
				public float fMoveSpeed = 1.0f;				// 이동 속도 (1.0 = 100%)
			}
