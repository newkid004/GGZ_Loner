...
namespace Habrador_Computational_Geometry
{
	...
	public static class _Geometry
	{
		...
		public static Vector2 CalculateCircleCenter(Vector2 a, Vector2 b, Vector2 c)
		{
			...
			//float L_10 = Vector2.Magnitude(b - a);
			//float L_20 = Vector2.Magnitude(c - a);

			//float L_10_square = L_10 * L_10;
			//float L_20_square = L_20 * L_20;

			float L_10_square = Vector2.SqrMagnitude(b - a);
			float L_20_square = Vector2.SqrMagnitude(c - a);
		...
		//2d
		public static float GetSignedDistanceFromPointToPlane(Vector2 pointPos, Plane2 plane)
		{
		...
		public static Vector2 GetClosestPointOnLine(Edge2 e, Vector2 p, bool withinSegment)
		{
			Vector2 a = e.p1;
			Vector2 b = e.p2;
			...
			//float aq_distance = Vector2.Dot(ap, ab) / Vector2.Magnitude(ab);

			//To get the closest point on the line:
			//Vector2 q = a + Vector2.Normalize(ab) * aq_distance;
		...
		public static Triangle2 GenerateSupertriangle(HashSet<Vector2> points)
		{
			//Step 1. Create a AABB around the points
			AABB2 aabb = new AABB2(new List<Vector2>(points));

			Vector2 TL = new Vector2(aabb.min.x, aabb.max.y);
			Vector2 TR = new Vector2(aabb.max.x, aabb.max.y);
			Vector2 BR = new Vector2(aabb.max.x, aabb.min.y);


			//Step2. Find the inscribed circle - the smallest circle that surrounds the AABB
			Vector2 circleCenter = (TL + BR) * 0.5f;

			float circleRadius = Vector2.SqrMagnitude(circleCenter - TR);
			...
			Vector2 t_B = new Vector2(circleCenter.x, circleCenter.y - circleRadius);

			Vector2 t_BL = new Vector2(t_B.x - halfSideLenghth, t_B.y);
			Vector2 t_BR = new Vector2(t_B.x + halfSideLenghth, t_B.y);

			//The height from the bottom edge to the top vertex
			float triangleHeight = halfSideLenghth * Mathf.Tan(60f * Mathf.Deg2Rad);

			Vector2 t_T = new Vector2(circleCenter.x, t_B.y + triangleHeight);
