...
namespace Habrador_Computational_Geometry
{
	...
	public static class SutherlandHodgmanModified
	{
		...
		public static List<List<Vector2>> ClipPolygonInverted(List<Vector2> poly, List<Plane2> clippingPlanes)
		{
		...
		//Assumes the polygon we want to remove from the other polygon is convex, so clipPolygon has to be convex
		//We will end up with the !intersection of the polygons
		public static List<List<Vector3>> ClipPolygonInverted(List<Vector3> poly, List<Plane2> clippingPlanes)
		{
			//The result may be more than one polygons
			List<List<Vector3>> finalPolygons = new List<List<Vector3>>();
			...
			for (int i = 0; i < clippingPlanes.Count; i++)
			{
				...
				//A new polygon which is the part of the polygon which is outside of this plane
				List<Vector3> outsidePolygon = new List<Vector3>();
		...
		//Assumes both polygons are convex unless you want the intersection, then only the clipping polygon
		//needs to be convex
		public static List<List<Vector3>> BooleanOperations(List<Vector3> poly, List<Vector3> clipPoly, BooleanOperation booleanOperation)
		{
			List<List<Vector3>> finalPolygon = new List<List<Vector3>>();
			...
			if (booleanOperation == BooleanOperation.Intersection)
			{
				List<Vector3> intersectionPolygon = SutherlandHodgman.ClipPolygon(poly, clipPoly);
