...
namespace Habrador_Computational_Geometry
{
	public static class TriangulateConvexHull
	{
		...
		public static HashSet<Triangle2> GetTriangles(List<Vector2> pointsOnHull, bool addColinearPoints)
		{
			...
			Vector2 a = pointsOnHull[0];

			//And then we just loop through the other edges to make all triangles
			for (int i = 1; i < pointsOnHull.Count; i++)
			{
				Vector2 b = pointsOnHull[i];
				Vector2 c = pointsOnHull[MathUtility.ClampListIndex(i + 1, pointsOnHull.Count)];
			...
			{
				HashSet<Vector2> colinearPoints = new HashSet<Vector2>(pointsOnHull);
				...
				foreach (Vector2 p in colinearPoints)
				{
					foreach (Triangle2 t in triangles)
					{
		...
		private static void SplitTriangleEdge(Triangle2 t, Vector2 p, HashSet<Triangle2> triangles)
		{
			Vector2 a = t.p1;
			Vector2 b = t.p2;
			Vector2 c = t.p3;
		...
		public static HashSet<Triangle2> GetTriangles(List<Vector2> points, Vector2 pointInside)
		{
			HashSet<Triangle2> triangles = new HashSet<Triangle2>();

			//This vertex will be a vertex in all triangles
			Vector2 a = pointInside;

			//And then we just loop through the other edges to make all triangles
			for (int i = 0; i < points.Count; i++)
			{
				Vector2 b = points[i];
				Vector2 c = points[MathUtility.ClampListIndex(i + 1, points.Count)];
