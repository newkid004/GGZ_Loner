...
namespace Habrador_Computational_Geometry
{
	...
	public static class EarClippingHoleMethods
	{
		...
		public static List<Vector2> MergeHolesWithHull(List<Vector2> verticesHull, List<List<Vector2>> allHoleVertices)
		{
			//Validate
			...
			foreach (List<Vector2> hole in allHoleVertices)
			{
				//Validate data
		...
		private static void MergeHoleWithHull(EarClippingPolygon hull, EarClippingPolygon hole)
		{
			...
			Vector2 lineStart = hole.maxX_Vert;
			//Just add some value so we know we are outside
			Vector2 lineEnd = new Vector2(hull.maxX_Vert.x + 0.01f, hole.maxX_Vert.y);
			...
			List<Vector2> verticesHull = hull.Vertices;
		...
		private static void FindVisibleVertexOnHUll(EarClippingPolygon hull, EarClippingPolygon hole, Edge2 line_hole_to_outside, out int closestEdge, out Vector2 visibleVertex)
		{
			//The first and second point on the hull is defined as edge 0, and so on...
			closestEdge = -1;
			//The vertex that should be visible to the hole (which is the max of the line that's intersecting with the line)
			visibleVertex = new Vector2(-1f, -1f);


			//Do line-line intersection to find intersectionVertex which is the point of intersection that's the closest to the hole
			Vector2 intersectionVertex = new Vector2(-1f, -1f);
			...
			{
				Vector2 p1_hull = verticesHull[i];
				Vector2 p2_hull = verticesHull[MathUtility.ClampListIndex(i + 1, verticesHull.Count)];
				...
				if (isIntersecting)
				{
					Vector2 testIntersectionVertex = _Intersections.GetLineLineIntersectionPoint(line_hole_to_outside, line_hull);

					//if (hole.id == 3) TestAlgorithmsHelpMethods.DebugDrawCircle(testIntersectionVertex.ToVector3(), 0.2f, Color.green);

					float distanceSqr = Vector2.SqrMagnitude(line_hole_to_outside.p1 - testIntersectionVertex);
			...
			Vector2 p1 = hull.Vertices[closestEdge];
			Vector2 p2 = hull.Vertices[MathUtility.ClampListIndex(closestEdge + 1, hull.Vertices.Count)];
			...
			{
				float hole_p1 = Vector2.SqrMagnitude(hole.maxX_Vert - p1);
				float hole_p2 = Vector2.SqrMagnitude(hole.maxX_Vert - p2);
		...
		private static void FindActualVisibleVertexOnHull(EarClippingPolygon hull, EarClippingPolygon hole, Vector2 intersectionVertex, ref Vector2 visibleVertex)
		{
			//Form a triangle
			...
			List<Vector2> reflectVertices = FindReflectVertices(hull, hole);
			...
			foreach (Vector2 v in reflectVertices)
			{
				if (_Intersections.PointTriangle(t, v, includeBorder: true))
				{
					...
					if (angle < minAngle)
					{
						...
						minDistSqr = Vector2.SqrMagnitude(v - hole.maxX_Vert);
					...
					{
						float distSqr = Vector2.SqrMagnitude(v - hole.maxX_Vert);
		...
		private static List<Vector2> FindReflectVertices(EarClippingPolygon hull, EarClippingPolygon hole)
		{
			List<Vector2> reflectVertices = new List<Vector2>();
			...
			for (int i = 0; i < verticesHull.Count; i++)
			{
				...
				Vector2 p_prev = verticesHull[MathUtility.ClampListIndex(i - 1, verticesHull.Count)];

				Vector2 p_next = verticesHull[MathUtility.ClampListIndex(i + 1, verticesHull.Count)];
