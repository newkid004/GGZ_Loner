...
namespace Habrador_Computational_Geometry
{
	...
	public static class CutMeshWithPlane 
	{
		...
		public static List<Mesh> CutMesh(Transform meshTrans, OrientedPlane3 orientedCutPlaneGlobal)
		{
			...
			Vector3 planePosLocal = meshTrans.InverseTransformPoint(cutPlaneGlobal.pos.ToVector3()).ToVector3();
			Vector3 planeNormalLocal = meshTrans.InverseTransformDirection(cutPlaneGlobal.normal.ToVector3()).ToVector3();
			...
			for (int i = 0; i < triangles.Length; i += 3)
			{
				...
				Vector3 p1 = p1_unity.ToVector3();
				Vector3 p2 = p2_unity.ToVector3();
				Vector3 p3 = p3_unity.ToVector3();

				//Normals
				Vector3 n1 = normals[triangleIndex1].ToVector3();
				Vector3 n2 = normals[triangleIndex2].ToVector3();
				Vector3 n3 = normals[triangleIndex3].ToVector3();
		...
		private static HashSet<Hole> FillHoles(HashSet<HalfEdge3> holeEdgesI, HashSet<HalfEdge3> holeEdgesO, OrientedPlane3 orientedCutPlane, Transform meshTrans, Vector3 planeNormal)
		{
			...
			foreach (List<HalfEdge3> hole in allHoles)
			{
				...
				List<Vector2> sortedEdges_2D = new List<Vector2>();
				...
				foreach (HalfEdge3 e in hole)
				{
					...
					Vector2 p2D = new Vector2(pPlaneSpace.x, pPlaneSpace.z);
				...
				foreach (Triangle2 t in triangles)
				{
					...
					//For inside mesh
					MyMeshVertex v1_I = new MyMeshVertex(p1Mesh.ToVector3(), planeNormal);
					MyMeshVertex v2_I = new MyMeshVertex(p2Mesh.ToVector3(), planeNormal);
					MyMeshVertex v3_I = new MyMeshVertex(p3Mesh.ToVector3(), planeNormal);

					//For inside mesh
					MyMeshVertex v1_O = new MyMeshVertex(p1Mesh.ToVector3(), -planeNormal);
					MyMeshVertex v2_O = new MyMeshVertex(p2Mesh.ToVector3(), -planeNormal);
					MyMeshVertex v3_O = new MyMeshVertex(p3Mesh.ToVector3(), -planeNormal);
		...
		private static void CutTriangleOneOutside(MyMeshVertex O1, MyMeshVertex I1, MyMeshVertex I2, HalfEdgeData3 newMeshO, HalfEdgeData3 newMeshI, HashSet<HalfEdge3> newEdgesI, HashSet<HalfEdge3> newEdgesO, Plane3 cutPlane)
		{
			...
			//Vector3 normal_F1B1 = Vector3.Slerp(F1.normal.ToVector3(), B1.normal.ToVector3(), percentageBetween_F1B1).ToVector3();
			//Vector3 normal_B2F1 = Vector3.Slerp(B2.normal.ToVector3(), F1.normal.ToVector3(), percentageBetween_B2F1).ToVector3();
		...
		private static void CutTriangleTwoOutside(MyMeshVertex O1, MyMeshVertex O2, MyMeshVertex I1, HalfEdgeData3 newMeshO, HalfEdgeData3 newMeshI, HashSet<HalfEdge3> newEdgesI, HashSet<HalfEdge3> newEdgesO, Plane3 cutPlane)
		{
			...
			//Vector3 normal_F2B1 = Vector3.Slerp(F2.normal.ToVector3(), B1.normal.ToVector3(), percentageBetween_F2B1).ToVector3();
			//Vector3 normal_B1F1 = Vector3.Slerp(B1.normal.ToVector3(), F1.normal.ToVector3(), percentageBetween_B1F1).ToVector3();
