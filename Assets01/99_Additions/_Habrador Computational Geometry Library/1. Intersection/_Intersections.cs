...
namespace Habrador_Computational_Geometry
{
	...
	public static class _Intersections
	{
		...
		//http://thirdpartyninjas.com/blog/2008/10/07/line-segment-intersection/		
		public static Vector2 GetLineLineIntersectionPoint(Edge2 a, Edge2 b)
		{
			float denominator = (b.p2.y - b.p1.y) * (a.p2.x - a.p1.x) - (b.p2.x - b.p1.x) * (a.p2.y - a.p1.y);

			float u_a = ((b.p2.x - b.p1.x) * (a.p1.y - b.p1.y) - (b.p2.y - b.p1.y) * (a.p1.x - b.p1.x)) / denominator;

			Vector2 intersectionPoint = a.p1 + u_a * (a.p2 - a.p1);
		...
		//2d
		private static Vector2 GetIntersectionCoordinate(Plane2 plane, Ray2 ray)
		{
			float denominator = Vector2.Dot(-plane.normal, ray.dir);

			Vector2 vecBetween = plane.pos - ray.origin;

			float t = Vector2.Dot(vecBetween, -plane.normal) / denominator;

			Vector2 intersectionPoint = ray.origin + ray.dir * t;
		...
		public static bool RayPlane(Plane2 plane, Ray2 ray)
		{
			...
			float denominator = Vector2.Dot(plane.normal * -1f, ray.dir);
		...
		//2d
		public static Vector2 GetRayPlaneIntersectionPoint(Plane2 plane, Ray2 ray)
		{
			Vector2 intersectionPoint = GetIntersectionCoordinate(plane, ray);
		...
		public static bool LinePlane(Plane2 plane, Edge2 line)
		{
			...
			Vector2 lineDir = (line.p1 - line.p2).normalized;
			...
			{
				Vector2 vecBetween = plane.pos - line.p1;

				float t = Vector2.Dot(vecBetween, -plane.normal) / denominator;

				Vector2 intersectionPoint = line.p1 + lineDir * t;
		...
		//2d
		public static Vector2 GetLinePlaneIntersectionPoint(Plane2 plane, Edge2 line)
		{
			Vector2 lineDir = (line.p1 - line.p2).normalized;
		...
		//2d
		public static Vector2 GetPlanePlaneIntersectionPoint(Plane2 plane_1, Plane2 plane_2)
		{
			Vector2 lineDir = (new Vector2(plane_2.normal.y, -plane_2.normal.x)).normalized;

			Ray2 ray = new Ray2(plane_2.pos, lineDir);

			Vector2 intersectionPoint = GetIntersectionCoordinate(plane_1, ray);
		...
		public static bool PointTriangle(Triangle2 t, Vector2 p, bool includeBorder)
		{
			//To avoid floating point precision issues we can add a small value
			float epsilon = MathUtility.EPSILON;
		...
		{
			//Center of circle
			Vector2 circleCenter = _Geometry.CalculateCircleCenter(a, b, c);

			//The radius sqr of the circle
			float radiusSqr = Vector2.SqrMagnitude(a - circleCenter);

			//The distance sqr from the point to the circle center
			float distPointCenterSqr = Vector2.SqrMagnitude(testPoint - circleCenter);
