	...
	public static class Grid
	{
		...
		public static HashSet<Triangle2> GenerateGrid(float width, int cells)
		{
			...
			for (int i = 0; i < verticesInOneRow; i++)
			{
				...
				{
					Vector2 vertexPos = new Vector2(-halfWidth + i * cellWidth, -halfWidth + j * cellWidth);
			...
			for (int i = 0; i < verticesInOneRow; i++)
			{
				for (int j = 0; j < verticesInOneRow; j++)
				{
					...
					else
					{
						...
						Vector2 BL = vertices[BL_pos];
						Vector2 BR = vertices[BR_pos];
						Vector2 TL = vertices[TL_pos];
						Vector2 TR = vertices[TR_pos];
