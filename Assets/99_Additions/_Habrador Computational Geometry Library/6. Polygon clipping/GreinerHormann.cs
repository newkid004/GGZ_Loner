...
namespace Habrador_Computational_Geometry
{
	...
	public static class GreinerHormann
	{
			...
			for (int i = 0; i < poly.Count; i++)
			{
				...
				for (int j = 0; j < clipPoly.Count; j++)
				{
					...
					if (_Intersections.LineLine(a_b, c_d, true))
					{
						...
						Vector2 intersectionPoint2D = _Intersections.GetLineLineIntersectionPoint(a_b, c_d);
		...
		private static ClipVertex InsertIntersectionVertex(Vector2 a, Vector2 b, Vector2 intersectionPoint, ClipVertex currentVertex)
		{
			...
			//pos = start + dir * alpha
			float alpha = Vector2.SqrMagnitude(a - intersectionPoint) / Vector2.SqrMagnitude(a - b);
