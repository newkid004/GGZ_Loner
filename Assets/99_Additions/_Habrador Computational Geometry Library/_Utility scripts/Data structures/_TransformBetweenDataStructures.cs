...
namespace Habrador_Computational_Geometry
{
	...
	public static class _TransformBetweenDataStructures
	{
		...
		public static HashSet<Triangle2> MeshToTriangle2(Vector2[] meshVertices, int[] meshTriangles)
		{
			...
			{
				Vector2 v1 = new Vector2(meshVertices[meshTriangles[i + 0]].x, meshVertices[meshTriangles[i + 0]].y);
				Vector2 v2 = new Vector2(meshVertices[meshTriangles[i + 1]].x, meshVertices[meshTriangles[i + 1]].y);
				Vector2 v3 = new Vector2(meshVertices[meshTriangles[i + 2]].x, meshVertices[meshTriangles[i + 2]].y);
		...
		public static Mesh Triangles2ToMesh(HashSet<Triangle2> triangles, bool useCompressedMesh, float meshHeight = 0f)
		{
			...
			foreach (Triangle2 t in triangles)
			{
				triangles_3d.Add(new Triangle3(t.p1.ToVector3_Yis3D(meshHeight), t.p2.ToVector3_Yis3D(meshHeight), t.p3.ToVector3_Yis3D(meshHeight)));
			}
