...
namespace Habrador_Computational_Geometry
{
	...
	public static class MarchingSquares
	{
		...
		public static SquareGrid GenerateMesh(float[,] map, float squareSize, bool shouldSmooth)
		{
			...
			squareGrid.vertices = new List<Vector2>(vertices);
		...
		{
			Vector2 R_inter = GetLerpedMidpoint(square.TR.pos, square.TR.value, square.BR.pos, square.BR.value);
			Vector2 B_inter = GetLerpedMidpoint(square.BR.pos, square.BR.value, square.BL.pos, square.BL.value);
			Vector2 L_inter = GetLerpedMidpoint(square.BL.pos, square.BL.value, square.TL.pos, square.TL.value);
			Vector2 T_inter = GetLerpedMidpoint(square.TL.pos, square.TL.value, square.TR.pos, square.TR.value);
		...
		private static Vector2 GetLerpedMidpoint(Vector2 v1, float weight1, Vector2 v2, float weight2)
		{
			float weight = (1f - weight1) / (weight2 - weight1);

			//Vector2 interpolatedPos =  v1 + (v2 - v1) * weight;

			Vector2 interpolatedPos = BezierLinear.GetPosition(v1, v2, weight);

			//float interX = _Interpolation.Sinerp(v1.x, v2.x, weight);
			//float interY = _Interpolation.Sinerp(v1.y, v2.y, weight);

			//Vector2 interpolatedPos = new Vector2(interX, interY);
