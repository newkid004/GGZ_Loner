...
namespace Habrador_Computational_Geometry
{
	...
	{
		public static HashSet<Triangle2> TriangulatePoints(HashSet<Vector2> pointsHashset)
		{
		...
		private static HashSet<Triangle2> TriangulatePointsConvexHull(HashSet<Vector2> points)
		{
			if (points.Count < 3)
			{
			...
			HashSet<Vector2> triangulatePoints = new HashSet<Vector2>();
			...
			List<Vector2> pointsOnHull = _ConvexHull.JarvisMarch_2D(triangulatePoints);

			//Add the other points one-by-one
			foreach (Vector2 pointToAdd in sortedPoints)
			{
				...
				{
					Vector2 hull_p1 = pointsOnHull[j];
					Vector2 hull_p2 = pointsOnHull[MathUtility.ClampListIndex(j + 1, pointsOnHull.Count)];
				...
				if (couldFormTriangle)
				{
					...
					pointsOnHull = _ConvexHull.JarvisMarch_2D(new HashSet<Vector2>(pointsOnHull));
				}
				else
				{
