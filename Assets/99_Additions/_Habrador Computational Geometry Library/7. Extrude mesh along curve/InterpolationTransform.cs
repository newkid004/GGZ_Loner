...
namespace Habrador_Computational_Geometry
{
	...
	public class InterpolationTransform
	{
		...
		//Vector3 normal = new Vector3(-forwardDir.z, 0f, forwardDir.x);
		...
		public static MyQuaternion GetOrientation_RotationFrame(Vector3 position, Vector3 tangent, InterpolationTransform previousTransform)
		{
			...
			Vector3 v1 = position - previousTransform.position;

			float c1 = Vector3.Dot(v1, v1);

			Vector3 riL = previousTransform.Right - v1 * (2f / c1) * Vector3.Dot(v1, previousTransform.Right);

			Vector3 tiL = previousTransform.Forward - v1 * (2f / c1) * Vector3.Dot(v1, previousTransform.Forward);
		...
		//If input is Vector3.Right * 2f then we should get a point in world space on the curve 
